import { GoogleGenAI, FunctionDeclaration, Tool, Type, Content, Part } from "@google/genai";
    ...
    functionDeclarations: [
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            offset: { type: Type.INTEGER, description: "Offset para paginación" }
          },
          required: []
        }
      },
...
export class GeminiAgent {
  ...
  async sendMessage(history: string[], userMessage: string) {
    const chat = this.ai.chats.create({
      ...
      config: {
        systemInstruction: "Eres un asistente de ventas. Usa herramientas para pedir info de productos.",
        tools: tools
      },
      history: history.map(h => ({ role: "user", parts: [{ text: h }] }) as Content)
    });
    ...
    if (response.candidates?.[0]?.content?.parts?.[0]?.functionCall) {
      ...
      if (funcCall.name === "getProducts") {
        ...
        const { data } = await axios.get(`${this.backendUrl}/products?q=${query}&limit=${limit}&offset=${offset}`);
        result = data;
      }
      ...
      response = await chat.sendMessage({ message: [{ functionResponse: { name: funcCall.name, response: result, id: funcCall.id } }] as Part[] });
    }
